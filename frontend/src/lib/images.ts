...
// Banners are saved under `.data/banners/` and served via the
// `/api/banners/[file]` route. We deliberately do NOT use `public/banners/`
// because Next 16's production build snapshots the public directory at
// `next build` time — any file written there afterwards (i.e. every banner
// generated by the running app) 404s in production. `.data/` is the same
// directory the SQLite DB lives in, so a single persistent volume mount
// covers both DB and banners.
...
function buildImagePrompt(input: GenerateBannerInput): string {
  ...
  // Style invariants only — NOT subject, NOT palette, NOT lighting mood.
  // Subject is dictated by the title; palette + lighting + environment vary
  // by topic so we don't end up with thirty banners that all look like the
  // same dark-blue factory shot. The image-prompt-agent (gpt-4.1) does the
  // topic-specific scene selection; this seed only sets the photographic
  // medium and rules out illustration / CGI.
  return [
    `Photorealistic documentary-style color photograph. Hero image for an enterprise B2B industrial AI / IIoT article titled "${input.title}".`,

    desc,

    "Shot on a full-frame DSLR with a 35mm or 50mm prime lens. Camera settings, focal length, and depth of field should fit the scene — close-up macro work calls for a different look than an environmental wide.",

    "Subject must be CONCRETE and SPECIFIC to the post topic. Do NOT default to 'engineer in hard hat next to machinery'. Pick the framing the topic calls for — macro detail of one piece of equipment, mid-range of a working component, environmental wide of an operating space, top-down, low-angle through machinery, exterior of a facility — choose based on what the title is actually about.",

    "Lighting and palette must match the real environment of the chosen scene — do NOT force a uniform 'moody dark-blue factory' look on every image. Examples: a clean room is bright white and pale green; a refinery at dusk is warm orange and red; a server room is balanced cool white with status LED accents; a wind farm is open daylight blue sky; an offshore platform is steel grey with sea spray; a cement plant is dusty beige and warm light. Choose what fits.",

    "Real colors, real materials, real surfaces — brushed steel, concrete, paint, glass, fabric, rubber, exposed cabling. No neon glow, no oversaturation, no rainbow color grading.",

    "Composition: clean, editorial, strong focal hierarchy, rule-of-thirds, ample negative space — but vary across posts (sometimes centered, sometimes left-weighted, sometimes minimal).",
...
3. Vary the LOOK across posts. We do NOT want every banner to be the same moody dark-blue factory shot. Pick the lighting + palette + environment that fits the title:
   - clean room / lab        → bright white, pale green or beige, even fluorescent / overhead lighting
   - refinery / dusk plant   → warm orange + deep teal, low golden-hour sun, steel and pipe silhouettes
   - data center / server room → cool white + accent blue/green LED, controlled office light
   - wind farm / solar field → open daylight, big sky, soft blue + green + white turbines
   - cement / steel mill     → warm dusty beige + amber, harsh shadows from hot machinery
   - food / pharma plant     → stainless steel + bright clinical white, hygienic feel
   - offshore platform / oil rig → steel grey, sea spray, overcast sky, weathered surfaces
   - control room / SCADA wall → dim ambient + glow of monitors, blue/green/amber light
   - outdoor utility / substation → daylight, concrete, transformers, blue sky or overcast
   Each post is a fresh visual — DO NOT carry over the look of previous banners.

4. The photograph should look like a real working environment captured by a documentary photographer, not staged.

# Style invariants (always include)

- Photorealistic documentary photograph; NOT illustration, NOT 3D render, NOT CGI.
- Real-world lighting and palette derived from the chosen environment (see rule 3) — NOT a forced color scheme.
- 16:9 framing.
...
async function pexelsInlineImage(
  ...
  return { url: `/api/banners/${filename}`, alt, photographer: photo.photographer };
}
  ...
  const handle = async (
    ...
    try {
      ...
      if (
        ...
        out.url.startsWith("/api/banners/")
      ) {
        try {
